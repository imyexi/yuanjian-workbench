...
function selectedInsightReport(){return (state.project.ai_reports||[]).find(x=>x.id===workflow.reportChoices[state.project.id]&&x.kind==='insights')||[...(state.project.ai_reports||[])].reverse().find(r=>r.report?.modules?.segments?.status==='success'||r.report?.modules?.audience?.status==='success')||latestAI('insights')}
...
function audienceWorkspace(){
 ...
 const generate=button(rows.length?'更新人群与方案':'生成拆分与策略','generate-segments','','primary',activeAI()?'disabled':'');
 let body='';
 if(!rows.length)body=`<section class="research-preflight"><div><span class="eyebrow">研究准备 · 尚无人群分析结果</span><h2>把词背后的人，和他们为什么买，分清楚</h2><p>资料 → 场景与动机 → 人群差异 → 专属营销方案。已有数据直接沿用，不必重采，也不用先填六套表单。</p><div class="material-ledger">${[['关键词',state.project.keywords.length,'发现搜索任务'],['原帖',(state.project.posts||[]).length,'还原发生场景'],['评论',state.project.reviews.length,'核对顾虑与期待']].map(([title,n,desc])=>`<div><strong>${n}</strong><span>${title}</span><small>${desc}</small></div>`).join('')}</div><div class="actions">${generate}${button('补充原帖 / 评论','navigate','','','data-page="research"')}</div><p class="caption">点击生成才发送资料到模型服务；关键词须先清洗；原帖和评论可独立分析，人群推断仍需验证。</p></div><aside><h3>生成后，你将看到什么</h3><ol><li><strong>人群对照表</strong><p>同一行为下，不同人的购买理由、人生阶段和决策障碍。</p></li><li><strong>逐人群深描</strong><p>发生场景、显性与深层需求、触发因素、替代办法和原始依据。</p></li><li><strong>可讨论的营销方案</strong><p>产品价值、渠道动作、内容方向、达人条件与促销测试。</p></li></ol></aside></section><section class="report-sheet"><h2>当前资料预览</h2><p class="caption">以下是当前项目原词，不是模型推断的人群。</p><div class="word-preview">${state.project.keywords.slice(0,16).map(x=>`<span>${esc(x.text)}</span>`).join('')||'<p>尚未导入关键词。</p>'}</div>${button('打开词库检查来源','navigate','','small','data-page="keywords"')}</section>`;
 else if(page==='audience')body=`<section class="report-sheet"><div class="section-head"><div><span class="eyebrow">人群差异总览 · ${rows.length} 类候选</span><h2>同一个行为，不同的购买理由</h2></div></div><div class="table-wrap"><table class="audience-matrix"><thead><tr><th>候选人群</th><th>购买动机</th><th>人生阶段 / 状态</th><th>发生场景</th><th>关键障碍</th><th>继续分析</th></tr></thead><tbody>${rows.map((x,i)=>`<tr><th>${esc(x.name)}</th><td>${esc((x.buying_motivation||x.needs?.implicit)?.text||'待验证')}</td><td>${esc((x.life_stage||x.layers?.social)?.text||'待验证')}</td><td>${esc(x.layers?.scene?.text||'待验证')}</td><td>${esc(x.barriers?.[0]?.text||'待验证')}</td><td>${button('查看方案','choose-audience','','small',`data-index="${i}"`)}</td></tr>`).join('')}</tbody></table></div><p class="caption">这是候选假设对照，不是市场人口分布；逐项证据与推断标记见下方深描。</p></section><p class="source-note">先看差异，再选人群。同一个“送礼”标签，可以是仪式展示、关系表达，也可以是长期纪念；这些只是划分方法示例，不是当前项目结论。</p><div class="audience-candidates">${rows.map((x,i)=>`<article class="report-sheet"><span class="eyebrow">候选 ${i+1} · ${esc(x.priority)}</span><h2>${esc(x.name)}</h2><p>${esc(x.one_line)}</p>${field('发生在什么场景',x.layers?.scene)}${field('为什么买',x.buying_motivation||x.needs?.implicit)}${field('人生阶段 / 当前状态',x.life_stage||x.layers?.social)}<details class="report-details"><summary>展开需求、购买决策与替代方案</summary>${field('明确想解决什么',x.needs?.explicit)}${field('更深层的期待 · 待核对',x.needs?.deep)}${(x.purchase_triggers||[]).map(v=>field('什么时候开始考虑购买',v)).join('')}${(x.decision_factors||[]).map(v=>field('买之前比较什么',v)).join('')}${(x.alternatives||[]).map(v=>field('现在用什么办法替代',v)).join('')}</details><h3>对应原词与证据</h3>${(x.search_terms||[]).map(t=>`<p>${esc(t.text)} ${evidenceButton([t.evidence_id],'查看原词',r.id,module)}</p>`).join('')}${evidenceButton(x.evidence_ids||[],'查看依据',r.id,module)}${button('看这类人的营销策略 →','choose-audience','','primary',`data-index="${i}"`)}</article>`).join('')}</div>`;
 else body=`<div class="strategy-workbench"><aside class="person-rail"><div class="rail-heading"><span class="eyebrow">候选人群</span><h2>为谁制定方案</h2><p>选中一类，右侧全部内容随之更新。</p></div>${rows.map((x,i)=>`<button class="person-choice ${i===index?'selected':''}" aria-pressed="${i===index}" data-action="choose-audience" data-index="${i}"><span>人群 ${String(i+1).padStart(2,'0')} · ${esc(x.priority)}</span><strong>${esc(x.name)}</strong><small>${esc(x.one_line)}</small></button>`).join('')}${button('返回横向对照','navigate','','small','data-page="audience"')}</aside><div class="strategy-detail"><div class="audience-context"><label>当前人群 <select id="strategy-audience">${rows.map((x,i)=>`<option value="${i}" ${i===index?'selected':''}>${esc(x.name)}</option>`).join('')}</select></label><p>${esc(person.one_line)}</p></div><section class="report-sheet"><h2>先确定：给谁、解决什么、凭什么</h2><div class="tower-grid">${field('购买动机',person.buying_motivation||person.needs?.implicit)}${field('价值主张',person.positioning?.promise)}${field('产品证明',person.positioning?.proof)}${field('不应承诺什么',person.positioning?.avoid)}</div></section><section class="report-sheet"><h2>这类人的营销方案</h2><p class="caption">方案跟随当前人群切换。所有产品设计、渠道和优惠建议都需实际验证，不代表已有能力或效果。</p>${[['产品策略：特性 / 设计 / 价值','product'],['渠道：在哪里找到这类人','channels'],['内容：痛点 / 场景 / 情感','content'],['达人：谁来讲更可信','creators'],['促销：组合 / 门槛 / 测试','promotion']].map(([title,k])=>field(title,person.marketing?.[k])).join('')}</section><details class="report-details"><summary>展开分析依据与待验证问题</summary>${field('人群优先级依据',person.priority_reason)}${field('当前情绪',person.layers?.emotion)}${(person.barriers||[]).map(x=>field('决策障碍',x)).join('')}<ul>${(person.validation_questions||[]).map(q=>`<li>${esc(q)}</li>`).join('')}</ul>${evidenceButton(person.evidence_ids||[],'查看原始依据',r.id,module)}</details><section class="report-sheet"><h2>场景与决策过程</h2><div class="scene-timeline">${(person.day_in_life||[]).map(x=>`<article><span class="eyebrow">${esc(x.moment)}</span><h3>${esc(x.scene)}</h3><p>想完成：${esc(x.task)}</p>${point({...x,text:'卡住的地方：'+x.friction})}</article>`).join('')}</div><div class="tower-grid">${(person.purchase_triggers||[]).map(x=>field('购买触发',x)).join('')}${(person.alternatives||[]).map(x=>field('当前替代办法',x)).join('')}</div></section></div></div>`;
 return head(page==='audience'?'拆分人群':'分人群策略',page==='audience'?'不是按身份贴标签，而是解释：同一个词下面，为什么有不同的购买理由。':'一次只看一类人，让产品、渠道、内容和促销围绕同一个购买动机。',button('历史报告 / 分析设置','navigate','','small','data-page="insights"'))+flowNav(page)+`<details class="report-details"><summary>已接入：消费动机洞察 · 本地资料分析</summary><p>情境：环境、身份/场景、个人属性；推力：功能问题、情绪状态、社交困扰；拉力：功能期望、情感期望、社交期望。</p><p>点击生成时用于分析当前项目资料，再输出分人群策略。未连接原 Skill 私有数据库；历史报告不会自动重写。</p></details>`+demoNote()+(activeAI()?aiProgress():'')+(ctx?reportMeta(ctx):'')+(ctx?.report?.limitations?.length?`<p class="source-note">资料缺口：${ctx.report.limitations.map(esc).join('；')}</p>`:'')+body+(rows.length?`<div class="actions">${generate}${button('返回词库补证据','navigate','','','data-page="keywords"')}</div>`:'');
}
const MOTIVE_FIELDS=[['environment','情境 · 环境'],['identity','情境 · 身份 / 场景'],['attributes','情境 · 个人属性'],['functional_problem','推力 · 功能问题'],['emotion','推力 · 情绪状态'],['social_friction','推力 · 社交困扰'],['functional_expectation','拉力 · 功能期望'],['emotional_expectation','拉力 · 情感期望'],['social_expectation','拉力 · 社交期望']];
function inputManifestWorkspace(){
 const p=state.project,clean=latestKeywordClassification(true),classes=keywordClassifications(clean),rows=p.keywords.map(k=>({k,c:classes.get(k.id)})),counts={included:0,excluded:0,pending:0};
 rows.forEach(({c})=>counts[c?c.valid?'included':'excluded':'pending']++);
 return head('分析资料清单','查看下一次分人群策略的关键词准入状态；清洗不删除原始资料。',button('更新清洗结果','navigate','','primary','data-page="keywords"'))+demoNote()+`<section class="report-sheet"><h2>纳入 ${counts.included} · 排除 ${counts.excluded} · 待清洗 / 复核 ${counts.pending}</h2><p>仅纳入与当前原词和数据版本一致的有效词。原帖 ${p.posts?.length||0} 篇、评论 ${p.reviews.length} 条独立取样；最终模型输入仍受样本与长度上限限制，不代表全量覆盖。</p><div class="table-wrap"><table><thead><tr><th>原词</th><th>来源</th><th>准入状态</th><th>理由</th></tr></thead><tbody>${rows.map(({k,c})=>`<tr><td>${esc(k.text)}</td><td>${esc(k.platform||'未注明')}</td><td>${c?c.valid?'纳入':'排除':'待清洗 / 复核'}</td><td>${esc(c?.reason||'没有匹配当前版本的完整清洗判定')}</td></tr>`).join('')||'<tr><td colspan="4">暂无关键词，可先导入或采集。</td></tr>'}</tbody></table></div></section>`;
}
function evidenceLedgerWorkspace(){
 const r=selectedInsightReport(),{rows,module}=audiencePerson(r),ctx=rows.length?insightModuleContext(r,module):null;
 const snapshot=new Map((ctx?.evidence_snapshot||[]).map(e=>[e.id,e]));
 const body=rows.flatMap(person=>MOTIVE_FIELDS.flatMap(([key,label])=>{const v=person.motives?.[key];if(!v)return [];const evidence=(v.evidence_ids||[]).map(id=>snapshot.get(id)).filter(Boolean);return [`<tr><th>${esc(person.name)}</th><td>${label}</td><td>${esc(v.text)}</td><td>${esc(({evidence:'原文表达',inference:'分析推断',unknown:'待验证'})[v.basis]||'未标注')}</td><td>${evidence.map(e=>`<p><strong>${esc(e.id)}</strong> · ${esc(e.platform)}<br>${esc(e.text)}</p>`).join('')||'暂无原文依据'}${evidenceButton(v.evidence_ids||[],'核对来源',r.id,module)}</td><td>${esc(v.validation||'未填写')}</td></tr>`]})).join('');
 return head('需求洞察库','逐条核对“原始摘录 → 动机判断 → 对应人群”。原始摘录是分析输入，不冒充模型逐字引语。',button('查看人群策略','navigate','','primary','data-page="strategy"'))+demoNote()+(ctx?reportMeta(ctx):'')+`<section class="report-sheet"><div class="table-wrap"><table><thead><tr><th>对应人群</th><th>分析维度</th><th>需求判断</th><th>依据性质</th><th>原词 / 原帖 / 评论摘录</th><th>如何验证</th></tr></thead><tbody>${body||'<tr><td colspan="6">尚无独立动机记录。补充资料并生成新版分人群策略后展示；旧报告不补造证据。</td></tr>'}</tbody></table></div></section>`;
}
function skillWorkspace(){return head('数据源与 Skill','查看已接入的方法与数据能力；配置或浏览不会自动开始采集。',button('查看数据源配置','sources','','primary'))+`<section class="report-sheet"><span class="eyebrow">项目级分析方法 · 已接入生成流程</span><h2>消费动机洞察</h2><p>情境：环境、身份/场景、个人属性。</p><p>推力：功能性问题、情绪状态、社交困扰。</p><p>拉力：功能性期望、情感期望、社交期望。</p><p>用于当前项目资料的人群拆分与策略生成；原始私有检索数据库未接入。旧报告保留原分析方法，不因接入而重写。</p></section>`}
function audienceLensWorkspace(){
 const page=state.page,r=selectedInsightReport(),{rows,index,module}=audiencePerson(r),person=rows[index],ctx=person?insightModuleContext(r,module):null;
 const title={nine:'消费动机洞察',tower:'人群深描与决策障碍',content:'内容方向与选题'}[page];
 const point=v=>v?insightNodesHtml([{type:'point',value:v}],ctx):'<p class="caption">当前报告未单列此项，待补证据或重新分析。</p>';
 const section=(label,v)=>`<section class="strategy-field"><h3>${label}</h3>${point(v)}</section>`;
 let body='';
 if(!person)body=empty('尚无人群分析结果','先从现有资料生成人群拆分与策略，再按不同视角查看；本页不补造分析。',button('前往人群分析','navigate','','primary','data-page="audience"'));
 else{
 body=`<div class="audience-context"><label>当前人群 <select id="strategy-audience">${rows.map((x,i)=>`<option value="${i}" ${i===index?'selected':''}>${esc(x.name)}</option>`).join('')}</select></label><p>${esc(person.one_line)}</p></div>`+reportMeta(ctx);
 if(page==='nine')body+=`<p class="source-note">每项独立保存依据和验证方法；旧报告未分析的字段保持未知，不从其他文字自动补齐。</p><div class="tower-grid">${MOTIVE_FIELDS.map(([k,label])=>section(label,person.motives?.[k])).join('')}</div>`;
 if(page==='tower')body+=`<div class="tower-grid">${[['基本属性','natural'],['社会身份','social'],['消费模式','consumption'],['行为场景','scene'],['生活方式','lifestyle'],['情绪状态','emotion']].map(([label,k])=>section(label,person.layers?.[k])).join('')}</div><section class="report-sheet"><h2>决策障碍与购买触发</h2>${(person.barriers||[]).map(point).join('')}${(person.purchase_triggers||[]).map(point).join('')}<h3>深层情感与价值观 · 原报告字段</h3>${point(person.layers?.deep_emotion)}${point(person.layers?.values)}<p class="caption">隐秘欲望没有独立证据字段，不由情绪自动推定。</p></section>`;
 if(page==='content')body+=`<section class="report-sheet"><h2>当前人群的内容方向</h2>${point(person.marketing?.content)}<h3>达人表达与筛选</h3>${point(person.marketing?.creators)}<h3>内容需要的证明</h3>${point(person.positioning?.proof)}</section>`;
 }
 if(page==='content'&&r?.report?.modules?.topics?.status==='success'){const c=insightModuleContext(r,'topics');body+=`<details class="report-details"><summary>项目级选题库 · 按面向人群核对</summary><p>以下是项目级建议，不自动视作当前人群的专属选题。</p>${insightNodesHtml(insightModuleNodes('topics',c.report,c),c)}</details>`}
 return head(title,'沿用同一份人群报告，分开查看洞察、深描和内容，不重复生成。',button('历史分析报告','navigate','','small','data-page="insights"'))+demoNote()+body;
}
...
function insightWorkspace(){
 ...
 return head('需求与选品','看清用户需要什么，再提出产品方向，用商品与竞品数据验证。',button('补充调研资料','navigate','arrow','','data-page="research"'))
  +flowNav('insights')+demoNote()+aiProgress()+(typeof recommendationHtml==='function'?recommendationHtml(r):'')
  ...
  +`<details class="analysis-settings" id="analysis-settings" data-reading-key="${esc((r?.id||'new')+'-setup')}" ${(workflow.analysisSettingsOpen[p.id]??!r)?'open':''}><summary>${r?'调整分析内容 / 生成新版':'选择本次分析内容'}</summary>${insightChoices()}</details>`
}
...
function insightModuleNodes(key,a,r){
 ...
 if(key==='clean'){
  ...
  nodes=[{type:'metrics',values:[['已分析关键词',s.total??a.items?.length??0],['有效词',s.valid??null,s.invalid===undefined?'':'待排除 '+s.invalid+' 条'],['归一主题',s.topic_count??null],['HOW 方法词',s.how_pct===undefined?null:s.how_pct+'%','占有效词']]},
   ...
   ...(x.motives?[group('消费动机独立记录',MOTIVE_FIELDS.map(([k,label])=>group(label,[point(x.motives[k])])))]:[]),
   ...(x.marketing?Object.entries(x.marketing).map(([k,v])=>group(({product:'产品策略',channels:'营销渠道',content:'内容营销',creators:'达人策略',promotion:'促销策略'})[k]||k,[point(v)])):[]),
...
function actionCards(interactive=true){
 const r=selectedInsightReport(),m=isModularInsight(r)?r.report.modules.summary:null;
 const context=m?.status==='success'?insightModuleContext(r,'summary'):null;
 const actions=context?.report?.actions||[];
 if(!actions.length)return `<section class="action-board"><div class="section-head"><div><span class="eyebrow">从判断到行动</span><h2>下一步行动</h2></div></div><p class="muted">尚未形成行动建议。先补充资料，再在需求与选品中选择「选品建议」。</p>${interactive?button('查看分析内容','view-insights','arrow','small'):''}</section>`;
...
function reportWorkspace(){
 const p=state.project,kw=latestKeywordClassification(),insight=selectedInsightReport();
 return head('报告与选品','优先推荐、商品比较与需求依据，都在这份报告里。',button('导出完整报告','export-report','download','primary')+button('导出项目包','export-project','download'))+demoNote()
  ...
  +(kw?`<div class="report-link-row"><div><h2>关键词分析</h2><p>${esc(kw.report.title||'已保存报告')}</p></div>${button('查看报告',kw.module_key==='clean'?'view-keyword-library':'view-keyword-report','arrow','',`data-report="${esc(kw.id)}"`)}</div><details class="report-reading-section" data-reading-key="${esc(kw.id+'-keywords')}"><summary>展开关键词分析</summary><div class="report-reading-content">${reportMeta(kw)}${keywordClassificationBody(kw)}</div></details>`:'')
...
async function startAI(kind,target='',ids=[],options={}){
 ...
 try{closeModal();if(kind==='keywords')goto('keywords');if(kind==='insights')goto(options.modules?.length===1&&options.modules[0]==='clean'?'keywords':options.modules?.includes('segments')?'audience':'insights');job=await api('/api/projects/'+projectId+'/ai',{revision:p.revision,kind,target,...(ids.length?{ids}:{}),...options});if(state.project?.id!==projectId)return;workflow.aiJob=job}
...
function keywordClassificationBody(r){
 if(r.module_key!=='clean')return keywordReportBody(r);
 const a=r.report||{};
 return `<h3>${esc(a.title||'关键词库')}</h3><p class="report-summary">${esc(a.summary)}</p>${a.quality==='limited'?'<p class="source-note">资料有限，请结合本项缺口阅读。</p>':''}${a.limitations?.length?`<section class="insight-limitations"><h3>本项资料缺口（${a.limitations.length}）</h3><ul>${a.limitations.map(x=>`<li>${esc(x)}</li>`).join('')}</ul></section>`:''}${insightNodesHtml(insightModuleNodes('clean',a,r),r)}`;
}
function fullReportHtml(){
 const p=state.project,k=latestKeywordClassification(),i=selectedInsightReport();
 const cleanInInsight=k?.module_key==='clean'&&i?.id===k.id&&isModularInsight(i)&&i.report.modules.clean?.status==='success';
 const evidenceId=k?'evidence-'+esc(k.id)+(k.module_key?'-'+esc(k.module_key):''):'';
 ...
 ${actionCards(false)}
 ${k&&!cleanInInsight?'<h2>关键词报告</h2>'+reportMeta(k)+keywordClassificationBody(k)+'<details class="report-appendix"><summary>关键词报告 · 原始证据快照</summary><div id="'+evidenceId+'">'+reportEvidenceHtml(k)+'</div></details>':k?'<h2>关键词报告</h2><p>'+esc(k.report.summary||'关键词库已包含在下方完整需求报告中。')+'</p>':'<p>关键词报告尚未生成。</p>'}
