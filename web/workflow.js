...
function insightWorkspace(){
 ...
 return head('需求与选品','看清用户需要什么，再提出产品方向，用商品与竞品数据验证。',button('补充调研资料','navigate','arrow','','data-page="research"'))
  +flowNav('insights')+demoNote()+aiProgress()+(r?`<details class="report-setup" data-reading-key="${esc(r.id+'-setup')}"><summary>调整分析范围 · 重新生成</summary>${insightChoices()}</details>`:insightChoices())
...
function insightNodesHtml(nodes,r,level=3){return nodes.map(n=>{
 ...
 if(n.type==='products')return `<section class="insight-part product-directions"><h3>值得验证的产品方向</h3><p class="caption">先从需求提出产品假设，再用商品数据核验竞争和机会。</p>${n.rows.length?n.rows.map((x,i)=>`<article class="product-direction"><div class="product-direction-heading"><span class="report-persona-index">${String(i+1).padStart(2,'0')}</span><div><h4>${esc(x.name)}</h4><p>面向：${esc(x.target)}</p></div><span class="persona-priority">待验证</span></div>${insightNodesHtml([{type:'group',title:'解决什么问题',children:[{type:'point',value:x.problem}]},{type:'group',title:'产品差异点',children:[{type:'point',value:x.differentiation}]}],r,level+1)}<div class="product-checks"><strong>接下来查什么</strong><ul>${(x.checks||[]).map(v=>`<li>${esc(v)}</li>`).join('')}</ul></div><p class="caption">建议检索词（AI 构造）：${(x.search_terms||[]).map(esc).join('、')}</p><div class="product-direction-actions">${button('去卖家精灵查商品','watch-product-direction','search','primary',`data-query="${esc((x.search_terms||[]).join('\n')||x.name)}"`)}${evidenceButton(x.evidence_ids,'查看需求依据',r.id,r.module_key)}</div></article>`).join(''):`<p class="caption">这份报告尚无具体产品方向。可按已有需求在电商盯盘继续验证，或重新生成「选品建议」。</p>${button('去卖家精灵查商品','watch-product-direction','arrow','small',`data-query="${esc(state.project.keyword||'')}"`)}`}</section>`;
...
case'export-ai-md':case'export-ai-html':{
 ...
 else download(r.kind+'-report.html',reportDocument(state.project.name,`${isModularInsight(r)?'':'<h1>'+esc(state.project.name)+'</h1>'+reportMeta(r)}${r.kind==='keywords'?keywordReportBody(r):insightReportBody(r,false)}${isModularInsight(r)?'':'<details class="report-appendix"><summary>原始证据快照</summary><div id="evidence-'+esc(r.id)+'">'+reportEvidenceHtml(r)+'</div></details>'}`),'text/html');
