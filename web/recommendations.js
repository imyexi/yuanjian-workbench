/* Saved product recommendations. Only an explicit action starts work; renderers and exports are read-only. */
...
function recommendationSource(report){
 ...
 if(isModularInsight(report)){const module=report.report.modules.summary;if(module?.status!=='success')return null;return {report_snapshot:{schema_version:2,report:module.report},evidence_snapshot:module.evidence_snapshot||[]}}
 if(report.kind==='insights'&&report.status==='success')return {report_snapshot:{schema_version:1,report:report.report},evidence_snapshot:report.evidence_snapshot||[]};
 return null;
}
function recommendationRecord(report,project=state.project){const source=recommendationSource(report);return source?[...(project?.product_recommendations||[])].reverse().find(x=>x.report_id===report.id&&JSON.stringify(x.report_snapshot)===JSON.stringify(source.report_snapshot)&&JSON.stringify(x.evidence_snapshot)===JSON.stringify(source.evidence_snapshot)):null}
...
function recommendationHtml(report,{exported=false}={}){
 ...
 }else body=`<p class="recommendation-empty" role="status">${esc(record?.status==='running'?phases[record.phase]||record.message:record?.status==='no_match'?result.no_match_reason||record.message:record?.message||entry?.error||eligible?'尚未生成商品推荐。':'请先完成选品建议模块，再生成商品推荐。')}</p>${record?.status==='success'&&!product?'<p class="error-inline">推荐商品快照缺失，暂不显示商品结论。</p>':''}`;
...
async function recommendationAction(action,el){
 if(action==='recommendation-start'||action==='recommendation-refresh'){const report=recommendationReport();if(!report||report.id!==el.dataset.report)return true;await recommendationsOnNavigation();return true}
