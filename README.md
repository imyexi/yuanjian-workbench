...
5. **从产品方向查竞品。**「选品建议」最多提出 3 个待验证方向，分别说明服务谁、解决什么问题、拟研究的差异点、依据和待查事项；证据不足时可以不推荐。点击「查找相关商品」只打开卖家精灵的 Amazon 查询弹窗并带入检索词，主动提交后才调用接口、消耗额度。
6. **验证并交付。**在「电商盯盘」关注和比较商品，手动读取价格、BSR、评分数和父子体月销量估算；在「报告与选品」整理选品依据、成本等资料缺口，导出 HTML、Markdown 报告或完整项目包。
...
HTML 报告内置样式和图表，可离线打开，保留可展开的完整画像、原始依据和来源；Markdown 保留相同章节和文字内容。完整项目包用于在另一份工作台中恢复资料与历史报告。

旧报告照常可读；没有产品方向的旧「综合判断」不会自动补写。需要新方向时，在对应报告中重跑「选品建议」；资料已经变化时，按页面提示使用当前资料重新生成。
