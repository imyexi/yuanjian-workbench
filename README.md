...
5. **直接看参考推荐。**「选品建议」完成后，自动用卖家精灵查询 Amazon 美国站商品，再由本机 Codex 对照需求原文选出一个优先参考商品和最多两个备选。每次最多查询两个英文产品词、每词十件商品；页面显示推荐理由、真实商品数据和待核查事项。证据不足或候选不匹配时会说明原因。
...
旧报告照常可读。首次阅读已完成但尚无推荐的需求洞察时，会基于该报告及其证据自动补齐商品推荐，不用重跑整份洞察。同一份报告及其内容复用已保存的推荐，刷新、切页和导出不会重复调用。失败或中断保留已查结果，只有主动点击重试才再次查询；新生成的报告有独立查询预算。
...
- 关键词、内容和评论采集仍由用户发起；完成选品洞察或首次阅读尚无推荐的旧需求报告时，会自动查询商品并生成推荐。演示项目和初始化验收不调用外部服务。海外平台的中文采集词翻译目前依赖 TikHub；推荐检索词、报告和原文翻译使用 Codex。
...
- `recommendation_jobs.py`、`web/recommendations.js`：有预算的自动商品查询、推荐缓存、原地对比和导出。
